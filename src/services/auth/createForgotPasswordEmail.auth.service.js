...
export async function createForgotPasswordEmail(userId, userEmail) {
	...
	if (otpCheckResult === true) {
		// return true
		const getUnexpiredOTPEmailDetails = await conn.query('SELECT otp_value, date_sent, date_expiration from tbl_users_otp WHERE users_id = $1;', [userId]);
		const unexpiredOTPDate = TZDate.tz("Asia/Karachi", getUnexpiredOTPEmailDetails.rows[0].date_expiration).internal.toISOString()
		const formattedUnexpiredTimestamp = formatDate(unexpiredOTPDate, 'PPPPpp').concat(" PKT")

		const unexpiredTimeDifferenceForHumans = formatDistance(unexpiredOTPDate.replace('T', ' ').replace('Z', ''), currentTimestampISO,
			{
				addSuffix: true,
				includeSeconds: true,
			}
