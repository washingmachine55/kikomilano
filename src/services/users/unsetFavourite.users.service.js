...
export async function deleteProductFavorite(userId, productVariantId) {
	...
	if (checkIfRecordExists === false) {
		const checkIfPreviouslyFavourited = await conn.query(
		...
		const reviouslyFavouritedCheck = checkIfPreviouslyFavourited.rows[0].existscheck;
		if (reviouslyFavouritedCheck === false) {
			conn.release();
