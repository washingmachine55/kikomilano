...
// ========================= New Orders ============================
const productsInOrder = z.strictObject({
	products_variants_id: UUIDSchema,
	qty: z.coerce.number("Quantity: Must be a valid number.")
		.max(20, "Quantity: Item count must be 20 or less")
		.min(1, "Quantity: You must have at least one item in cart to continue"),
}).required()

export const ordersNewJsonSchema = z.object({
	users_id: UUIDSchema,
	products: z.array(productsInOrder)
	...
	cart_total: z.coerce.number("Cart Total: Must be a valid number")
		.max(9999, "Cart Total: Total price can not exceed 9999"),
	discount_code: z.string("Discount code: Must be exactly 8 characters")
		.min(8)
		.max(8)
		.optional()
		.nullable(),
...
// ==================== User Profile Edit ==========================
export const userProfileEditJsonSchema = z.strictObject({
	...
	if (data.password !== undefined && data.password !== null && data.password !== '') {
		if (data.confirmed_password === undefined || data.confirmed_password === null || data.confirmed_password === '') {
			ctx.addIssue({
...
// ===================== User Set Favorite =========================
export const userFavoriteProductSchema = z.strictObject({
	users_id: UUIDSchema,
	products_variants_id: UUIDSchema,
...
const addressesBaseSchema = z.object({
	...
	zip_code: z.string().length(5).regex(/^\d{5}$/).nullable(),
